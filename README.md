...
## Highlights
1. First, our proposed approach requires only \textit{bag} level annotations (in contrast to many existing approaches using \textit{instance} level annotations), reducing the cost of labelling. Given a limited time-span, a pathologist can annotate multiple such WSIs. 
